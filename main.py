...
class AONet:
    ...
    def initialize(self, cuda_device=None):
        ...
        self.model = VASNet()
        self.model.eval() # by default, set to eval
...
def train(hps):
    # Create experiment directory and copy files in it
    ...
    # Logs: create a file to collect results from all splits
    f = open(os.path.join(hps.output_dir, 'results.txt'), 'wt')
    ...
    for split_filename in hps.splits:
        ...
        for split_id in range(n_folds):
            ...
            fscore, fscore_epoch = ao.train(hps.output_dir)
            f_avg += fscore
