...
class VASNetModel(Model):
    ...
    def train(self, fold):
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            self.hps.writer.add_scalar('{}/Train/Loss/Fold_{}'.format(self.hps.current_dataset, fold+1), train_avg_loss, epoch)

            # Evaluate performances on test keys
            if epoch % self.hps.test_every_epochs == 0 or epoch == 0:
                f_score = self.test(fold)
                self.model.train()
                self.hps.writer.add_scalar('{}/Test/F-score/Fold_{}'.format(self.hps.current_dataset, fold+1), f_score, epoch)
